			...
			d3.csv("static/data/data4.csv", function(error, data){
				...
				d3.selectAll('.rowLabel')
					...
					d3.selectAll('.cell')
						.on('mousedown', function(){
							var fill = this.getAttribute('style').split(':');
							if (fill[fill.length-1] != 'white'){
								var id = parseInt(this.getAttribute('id').split('cell')[1]);
								var str = this.parentNode.getAttribute('id');
								if (str.split('gr').length == 2){
									var ind = parseInt(str.split('gr')[1]);
								} else {
									var ind = parseInt(str.split('gc')[1]);
								} 
								if (orders[ind] < celldata.length/2){
									var color = 'red';
								} else var color = 'blue';
								var color2 = this.getAttribute('style').split(';')[0].split(' ')[1];
								if (color2 == 'none'){
									this.setAttribute('style', 'stroke: '+ color+' ; stroke-width: 10; opacity: 0.5; fill: black');
									if (str.split('gr').length == 2){
										d3.select('#gc'+id).select('#cell'+ind)
											.attr('style', 'stroke: '+ color+' ; stroke-width: 10; opacity: 0.5; fill: black');
										var style = d3.select('#gr'+id).select('#cell'+ind).attr('style').split(' ')[1]; 	
										if (style == color)	{
											d3.select('#line'+indices[id]+'_'+indices[ind]).attr('style', 'stroke :' + color + ';stroke-width: 4;');
											d3.select('#line'+indices[ind]+'_'+indices[id]).attr('style', 'stroke :' + color + ';stroke-width: 4;');											
										}

									}		
									else { 
										d3.select('#gr'+id).select('#cell'+ind)
											.attr('style', 'stroke: '+ color+' ; stroke-width: 10; opacity: 0.5; fill: black');
										var style = d3.select('#gc'+id).select('#cell'+ind).attr('style').split(' ')[1]; 	
										if (style == color)	{
											d3.select('#line'+indices[id]+'_'+indices[ind]).attr('style', 'stroke :' + color + ';stroke-width: 4;');
											d3.select('#line'+indices[ind]+'_'+indices[id]).attr('style', 'stroke :' + color + ';stroke-width: 4;');											
										}											
									...
									this.setAttribute('style', 'stroke: none; stroke-width: 10; opacity: 0.5; fill: black');
									if (str.split('gr').length == 2)
										d3.select('#gc'+id).select('#cell'+ind)
											.attr('style', 'stroke: none; stroke-width: 10; opacity: 0.5; fill: black');
									else 
										d3.select('#gr'+id).select('#cell'+ind)
											.attr('style', 'stroke: none; stroke-width: 10; opacity: 0.5; fill: black');
									d3.select('#line'+indices[id]+'_'+indices[ind]).attr('style', 'stroke : black; stroke-width: 4;');
									d3.select('#line'+indices[ind]+'_'+indices[id]).attr('style', 'stroke : black; stroke-width: 4;');
									if (color == 'red') redCount--;
									else blueCount--;									
								}
			...
			function swapRow(id, y){
				...
				var temp = orders[row];
				orders[row] = orders[newRow];
				orders[newRow] = temp;

				var temp = indices[row];
				indices[row] = indices[newRow];
				indices[newRow] = temp;
			...
			function swapColumn(id, x){
				...
				var temp = orders[column];
				orders[column] = orders[newColumn];
				orders[newColumn] = temp;		

				var temp = indices[column];
				indices[column] = indices[newColumn];
				indices[newColumn] = temp;						
			...
			function swapColumn2(column, newColumn){
			...
			function sort(){
				var left = [], right = [];
				for (var i = 0; i < celldata.length; i++){
					if (i < celldata.length/2){
						if (orders[i] >= celldata.length/2){
							left.push(i);
						}
					} else {
						if (orders[i] < celldata.length/2){
				...
				unsorted = orders;
				if (left.length != celldata.length/2)
				for (var i = 0; i < left.length; i++){
					swapColumn2(left[i],right[i])
				}
			...
			function unsort(){
				...
				var left2 = [], right2 = [];
				if (left.length == 0 || left.length == celldata.length/2){
					//for (var i = 0; i < orders.length; i++){
						
						swapColumn2(parseInt(Math.round(celldata.length/2)-1), 
								parseInt(Math.round(celldata.length/2))+1);
					//}
				} else {
					return;
				}			
