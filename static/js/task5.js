			...
			canvas.append('rect')
				...
				.attr('width', 425).attr('height', 350)
				.attr('style', 'stroke-width: 2; stroke: black; fill: none; opacity: 0.5');
			canvas.append('text')
				.attr('x', 660).attr('y', 75)
				.attr('class', 'noselect')
				.attr('style', 'fill: black; font-weight:900')
				.text('Example Network');

			// bounding box for the matrix visualization
			canvas.append('rect')
			...
			d3.csv("static/data/data2.csv", function(error, data){
				...
				var offsetx = 25;
				var offsety = 50;
				var svg2 = d3.select('#visualization').append('g').attr('transform', 'translate(275,100)');
