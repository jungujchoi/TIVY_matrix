			...
			statements[4] = names[ans[4][0]-1] + " and "+ names[ans[4][1]-1] + " are friends";				
				...
				for (var i = 0; i < fills.length; i++){
					...
					var c_x = circles[ans[fills[i]][0]-1][0];
					var c_y = circles[ans[fills[i]][0]-1][1];
					var c_x2 = circles[ans[fills[i]][1]-1][0];
					var c_y2 = circles[ans[fills[i]][1]-1][1];
					...
					/*svg.append('line')
						.attr('class', 'link')
						.attr('x1', circles[ans[fills[i]][0]-1][0]).attr('y1', circles[ans[fills[i]][0]-1][1])
						.attr('x2', circles[ans[fills[i]][1]-1][0]).attr('y2', circles[ans[fills[i]][1]-1][1])
					...
					svg.append('line')
						...
						.attr('style', 'stroke: black; stroke-width: 1px;');												
					//document.getElementById('cell(' + cx + ',' + cy + ')').setAttribute('style', 'fill:black');
					//document.getElementById('cell(' + cy + ',' + cx + ')').setAttribute('style', 'fill:black');
				}				
			...
			// bounding box for node-link diagram
			svg.append('rect')
				.attr('x', 175).attr('y', 20)
				.attr('width', 400).attr('height', 350)
				.attr('style', 'stroke:black; stroke-width: 2; fill: none; opacity: 0.5')
			svg.append('text')
				.attr('x', 185).attr('y', 40)
				.attr('class', 'noselect')
				.attr('style', 'fill: black; font-weight: 900;')
				.text('Node-link Diagram')			
			
			// bounding box for matrix visualization		
			svg.append('rect')
				.attr('x', 625).attr('y', 20)
				.attr('width', 400).attr('height', 350)
				.attr('style', 'stroke:black; stroke-width: 2; fill: none; opacity: 0.5')
			svg.append('text')
				.attr('x', 635).attr('y', 40)
				.attr('class', 'noselect')
				.attr('style', 'fill: black; font-weight: 900;')
			...
			d3.csv("static/data/data4.csv", function(error, data){
				...
				var canvas = svg.append('g').attr('class', 'canvas').attr('transform', 'translate(675,50)');
				...
				d3.selectAll('.cell').on('mousedown', function(){
					...
					if (ans[counter-1].includes(columnY) && ans[counter-1].includes(rowX) && columnY != rowX && ver.includes(id) == false){
						...
						if (counter != ans.length){
							var c_x = circles[ans[counter][0]-1][0];
							var c_y = circles[ans[counter][0]-1][1];
							var c_x2 = parseInt(d3.select('#circ'+circle).attr('cx'));
							var c_y2 = parseInt(d3.select('#circ'+circle).attr('cy'));
							var theta = Math.atan((c_y2-c_y)/(c_x2-c_x));
							var d_x = Math.abs(30*Math.cos(theta));
